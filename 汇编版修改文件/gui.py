...
class LexerApp:
    def __init__(self, master):
        ...
        self.input_text = tk.Text(input_frame, wrap=tk.NONE, undo=True, font=text_font,
                                  ...
                                  xscrollcommand=input_scroll_x.set, height=18)
...
fn main() -> i32 {
    ...
    return fact_x;
        ...
        self.ast_image_label.bind('<Configure>',
                                  lambda e: self.ast_canvas.configure(scrollregion=self.ast_canvas.bbox("all")))
        ...
        try:
            while True:
                ...
                if tok.type == TokenType.ILLEGAL:
                    output_lines.append(f"*** 错误: 非法字符 '{tok.literal}' ***");
                    error_found = True
        except Exception as e:
            ...
            if not semantic_errors_found:
                try:
                    ...
                    if quadruples:
                        ...
                        self.ir_table.insert('', 'end',
                                             values=('', '未生成中间代码。(可能是空程序或IR生成器未对所有情况处理)。\n'))
    ...
    def perform_assembly_generation(self):
        ...
        if self.ast_root_for_analysis is None:
            ...
            try:
                ...
                assembly_code = asm_gen.get_assembly()
                ...
                self.assembly_text.insert(tk.END, f"生成汇编代码时发生意外错误: {e}\n")
                import traceback
                self.assembly_text.insert(tk.END, traceback.format_exc())
    ...
    def assemble_link_and_run(self):
        ...
        try:
            ...
            output_dir = os.path.join(base_dir, "assembly_test")
