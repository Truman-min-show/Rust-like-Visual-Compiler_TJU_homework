...
class AssemblyGenerator(Visitor):
    ...
    def visit_for_statement(self, node: ForStatement):
        ...
        continue_label = self._new_label("for_continue") # 自增部分

        self.loop_start_labels.append(continue_label)
        self.loop_end_labels.append(end_label)
        
        # 1. 初始化循环变量
        # 语义分析阶段已经在新作用域内为循环变量分配了偏移
        var_name = node.variable.value
        offset = self.local_var_offsets[self.current_function_name].get(var_name)
        
        if offset is None:
            # 这是一个内部错误，语义分析应该已经创建了它
            print(f"Assembly Error: for loop variable '{var_name}' has no stack offset.")
            return

        # 计算起始值并存入变量
        node.iterator.left.accept(self) # result in rax
        self._add_line(f"mov [rbp {offset}], rax")

        # 计算结束值并保存在栈上，供每次循环比较
        node.iterator.right.accept(self) # result in rax
        self._add_line("push rax") # 保存结束值
        
        # 2. 循环开始
        self._add_line(f"{start_label}:", indent=False)
        
        # 3. 条件检查: i < end
        self._add_line(f"mov rax, [rbp {offset}]") # 加载 i
        self._add_line("cmp rax, [rsp]")          # 与栈顶的 end 比较
        self._add_line(f"jge {end_label}")        # if i >= end, jump to end
        ...
        self._add_line("inc rax") # add rax, 1
        self._add_line(f"mov [rbp {offset}], rax")
        ...
        self._add_line("add rsp, 8") # 清理栈上保存的结束值
