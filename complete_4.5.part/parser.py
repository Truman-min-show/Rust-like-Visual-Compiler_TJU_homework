...
class Parser:
    ...
    def _parse_for_statement(self) -> ForStatement | None:
        """解析 for 循环语句: for IDENT in Expression BlockStatement"""
        ...
        # 解析迭代器表达式 (例如 1..10)
        iterator_node = self.parse_expression(LOWEST)
        if iterator_node is None:
            return None
        ...
        return ForStatement(token=for_token, variable=variable_node, iterator=iterator_node, body=body_node)
