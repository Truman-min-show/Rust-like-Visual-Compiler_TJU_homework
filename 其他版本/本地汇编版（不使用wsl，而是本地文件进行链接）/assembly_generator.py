...
class AssemblyGenerator(Visitor):
    ...
    def visit_program(self, node: Program):
        self._add_line("extern ExitProcess", indent=False)
    ...
    def visit_function_declaration(self, node: FunctionDeclarationStatement):
            ...
            if not self.assembly_code or not self.assembly_code[-1].strip().startswith("jmp"):
                if self.current_function_name == "main":
                    self._add_line("mov rax, 0    ; Default exit code for main")
                    self._add_line("mov rcx, rax  ; Set exit code for ExitProcess")
                    self._add_line("call ExitProcess")
    ...
    def visit_return_statement(self, node: ReturnStatement):
        if node.return_value:
            node.return_value.accept(self)  # 返回值结果在 rax 中
        # --- 修改点: 如果在 main 函数中 return，则调用 ExitProcess ---
        if self.current_function_name == "main":
            self._add_line("mov rcx, rax  ; Set exit code for ExitProcess")
            self._add_line("call ExitProcess")
        else:
            # 对于其他函数，保持原有的跳转到函数末尾的逻辑
    ...
    def visit_call_expression(self, node: CallExpression):
        ...
        if len(node.arguments) > len(arg_regs):
            # 错误处理: 当前不支持超过4个参数的函数调用
            # 在实际编译器中这里应该报错
            return

        # Windows x64 ABI 要求调用者为被调用的函数提供32字节的“影子空间”
        # 并且在 call 指令执行时，栈指针 RSP 必须是16字节对齐的
        self._add_line("sub rsp, 32  ; Reserve shadow space for call")
        ...
        # 调用结束后，恢复为影子空间分配的栈空间
        self._add_line("add rsp, 32  ; Clean up shadow space")
